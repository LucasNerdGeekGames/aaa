window.addEventListener('load', function(){
        ...
        constructor(game){
            ...
            window.addEventListener('keyup', e =>{
                if (this.game.keys.indexOf(e.key) -1){
                    this.game.keys.splice(this.game.keys.indexOf(e.key), 3);
                }
            });
    ...
    class Player {
        ...
        shootTop(){
            ...
            this.projectiles.push(new Projectile(this.game, this.x + 42, this.y + 28));
            this.game.ammo--;
            this.frameX = 9.5 ;
            }
        }
        ...
        //    if (this.game.ammo > 0){
        //    // distance and local shoot
        //    this.projectiles.push(new Projectile2(this.game, this.x + 80, this.y + 130));
        //    this.game.ammo2--;
        //    }
        //}
    }
    ...
    class Game {
        ...
        update(deltaTime){
            ...
            this.enemies.forEach(enemy => {
                ...
                if (this.checkCollision(this.player, enemy)){
                    ...
                    //document.getElementById('smartphone').hidden = false;
                    this.player.x = 1195;
                    //window.location.href="jogo-e-fases2-hunt.html";
                }
            ...
            this.bossis.forEach(boss2 => {
                boss2.update();
                if (this.checkCollision(this.player, boss2)){
                    boss2.markedForDeletion = false;
                    this.score--;
                    this.gameTime += 5000;
                    // wall block here :) sair da cidade, mapa aberto visao de cima w,a,s,d.
                    //this.player.x = 1285;
                ...
                this.player.projectiles.forEach(projectile => {
                    if (this.checkCollision(projectile, boss2)){
                        boss2.lives--;
                        projectile.markedForDeletion = true;
                        if (boss2.lives <= 0){
                            boss2.markedForDeletion = false;
                            this.score += boss2.score;
                            if (this.score > this.winningScore) this.gameOver = true;
                        }
                    }
                })
            ...
            this.bossis = this.bossis.filter(enemy => !enemy.markedForDeletion);
            if (this.enemyTimer > this.enemyInterval && !this.gameOver){
                this.addBoss();
                // aqui tempo boss
        ...
        addBoss(){
            this.bossis.push(new Angler3(this));
            console.log(this.bossis);
        }
